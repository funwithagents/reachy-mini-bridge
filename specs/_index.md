...
| [config.md](config.md) | `ReachyMiniConfig` — one declarative config (backend, upstream `robot` kwargs, `daemon` management, tts-engine `tts` block, `audio` profile, `wobbling` flag and `motion` switches — on by default) buildable from a dict / JSON string / JSON file, mirroring tts-engine's `TTSEngineConfig` | Updated |
