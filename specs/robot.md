...
- **Motion / expression:** `async_play_move` (with `RecordedMoves` loaded at the api layer for `play_emotion`; the api cancels it and stops its sound itself — see [api.md](api.md) "Cancellation"). **The fake keeps the move's timing:** its `async_play_move` records the command, then sleeps for the move's `duration` (`0` for a move without one), so a cancel has something in flight to interrupt and a consumer testing turn-taking sees real completion. `start_head_tracking` / `stop_head_tracking`.
