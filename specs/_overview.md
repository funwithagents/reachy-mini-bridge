...
- **[`tts-engine`](../../tts-engine)** — our first-party streaming TTS engine, the **default** (but swappable) backend for `ReachyMiniApi.say` behind the bridge-owned `SpeechSynthesizer` interface. A base dependency (a small engine with no provider); its providers come through the `tts-pocket` (local model, no key) / `tts-elevenlabs` (cloud) extras, one per tts-engine extra. Referenced by its GitHub URL (tracking `main` until it settles, then pinned).
