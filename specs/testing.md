...
- **`sim`** (default) — the harness spawns and manages a MuJoCo daemon, in one of two **launch modes**:
  ...
  - **scene** (`REACHY_MINI_E2E_SIM_SCENE`, either mode): unset for upstream's default scene; an upstream scene name (`minimal`); a path to an `.xml` scene file; or `test` — the bridge's test scene, its props (a portrait plane today) hidden until a test shows, moves and hides them through the `sim_scene` fixture ([sim_scene.md](sim_scene.md)). Face tests gate on `camera` **and** `faces`, so they run on the viewer with the test scene and skip everywhere else.
...
| `faces` | the daemon's `/api/sim-scene/bodies` lists a `face` body — it runs the bridge's test scene ([sim_scene.md](sim_scene.md), `REACHY_MINI_E2E_SIM_SCENE=test`); the face starts hidden and a test shows it | ✅ scene loads (but nothing looks at it: no camera) | ✅ | ❌ |
