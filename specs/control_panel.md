...
- **Run it as a module** from the repo root, so the example package imports the same way from the command line and from `tests/` (`pyproject.toml` adds `.` to pytest's `pythonpath`):
  ...
  `--config PATH` is the JSON config file; without it the panel runs the `fake` backend. `config.example.json` is the sim **viewer** ([config.md](config.md)): the MuJoCo window opens next to the panel, so the robot's motion and its camera frame are both visible. `--host` / `--port` (default `127.0.0.1:7860`) place the web server. Ctrl+C in the terminal stops the server, then the controller closes the api session (which stops a daemon the bridge spawned).
