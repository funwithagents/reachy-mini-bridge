...
- **The adapter needs `tts-engine` to expose its PCM stream.** `tts-engine`'s `TTSEngine.speak` currently swallows audio into a local `sounddevice` player. The clean adapter wants a **sink abstraction** on `tts-engine` (a hand-off to be raised against that repo); until it lands, the adapter can drive `tts-engine`'s module layer directly (`load_module` + `module.stream(..., callback)`), at the cost of coupling to internals. `tts-engine` emits **int16** natively, so this adapter meets the `SpeechSynthesizer` contract via the bridge's public `int16_to_float32(...)` helper (see "Conversion helpers") — a conversion float-native synthesizers (most neural TTS) skip.
