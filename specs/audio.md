...
`media.start_recording()` / `stop_recording()`, `media.get_audio_sample()`, `media.get_input_audio_samplerate()`, `media.start_playing()` / `stop_playing()`, `media.push_audio_sample(np.float32)`, `media.audio.apply_audio_config(...)`, `media.audio.clear_player()` (barge-in flush), plus the format getters `media.get_input_channels()` / `get_output_audio_samplerate()` / `get_output_channels()`. (`media.get_DoA()` is not consumed yet — see open question 5.) The head wobbler is toggled on the robot object itself — `enable_wobbling()` / `disable_wobbling()` on `ReachyMini`, listed in [robot.md](robot.md)'s consumed slice — see "Head wobbling" below.
...
- **A `tts` block that fails to build degrades to no voice, not a fatal error.** The adapter's constructor is the usual failure point — it resolves the module's `api_key_env`, so an unset key (or a typo in `module.type`) raises there. The missing `tts` extra (`ImportError`) still raises `ConfigError` at construction, unchanged; every other exception is caught, exposed as `ReachyMiniApi.synthesizer_error`, and logged, and the api still comes up — see [api.md](api.md) "Constructed from a config" for the full rule.
...
- **The bridge exposes it as a mode, at the api layer.** `enable_wobbling()` / `disable_wobbling()` are methods of the robot object (they arm the SDK-side wobbler on the local media pipeline *and* the daemon-side one, which covers the daemon's own sounds), so the toggle is [api.md](api.md)'s `set_wobbling(enabled)` verb plus the top-level `wobbling` config flag ([config.md](config.md), on by default) applied on entry — not a `MediaSession` concern. It is specified here because it is a property of the speaker path: once on, every `say`, `play_sound` and emotion sound drives the head.
