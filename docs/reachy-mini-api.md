...
- **Composition order in the daemon** (`update_target_head_joints_from_ik`): the client's target head pose → blended toward the face-tracking aim by the tracking weight → speech (wobble) offsets composed on top → IK. At tracking weight **≥ 1.0** `set_target_head_pose` returns before flagging IK, so the head component of a `set_target` is **discarded** and only antennas / body yaw apply; below 1.0 the target shows through the blend. A lost face decays the aim back to `INIT_HEAD_POSE` after 2 s.
