...
_PKG = _REPO_ROOT / "src" / "reachy_mini_bridge"
_SPECS_DIR = _REPO_ROOT / "specs"

# Matches link targets like `src/reachy_mini_bridge/foo.py` anywhere in AGENTS.md.
_MODULE_LINK = re.compile(r"src/reachy_mini_bridge/([A-Za-z_][A-Za-z0-9_]*\.py)")
...
def test_agents_md_maps_exactly_the_package_modules():
    ...
    # Concept modules must be mapped; package glue (`__init__.py`) is exempt.
    missing_from_map = _concept_modules() - mapped
    stale_in_map = mapped - _actual_modules()
...
#     ---
#     code:
#       - src/reachy_mini_bridge/example.py
#     tests:
...
def test_every_concept_module_is_governed_by_a_spec():
    ...
    for spec in _spec_files():
        for rel in _parse_frontmatter(spec).get("code", []):
            if rel.startswith("src/reachy_mini_bridge/"):
